...
function TravelMapTab:Constructor(toplevel)
    ...
    self.regionNames = {
        [MapType.NONE] = LC.noneMap or "None",
        [MapType.CREEPS] = LC.moorMap or "Ettenmoors",
        [MapType.ERIADOR] = LC.eriadorMap or "Eriador",
        [MapType.RHOVANION] = LC.rhovanionMap or "Rhovanion",
        [MapType.ROHAN] = LC.rohanMap or "Rohan",
        [MapType.GONDOR] = LC.gondorMap or "Gondor",
        [MapType.HARADWAITH] = LC.haradwaithMap or "Haradwaith"
    };

    -- Create scrollable container for the map
    self.scrollWindow = Turbine.UI.ScrollableControl();
    self.scrollWindow:SetParent(self);
    self.scrollWindow:SetPosition(0, 0);
    self.scrollWindow:SetMouseVisible(true);

    -- Create the map display label
    self.mapLabel = Turbine.UI.Label();
    self.mapLabel:SetSize(self.mapWidth, self.mapHeight);
    self.mapLabel:SetParent(self.scrollWindow);
...
function TravelMapTab:SetSize(width, height)
    ...
    -- Update scroll window size (leave room for nav panel)
    self.scrollWindow:SetSize(width, height - 30);
